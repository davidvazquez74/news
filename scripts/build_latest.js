...
function normalizeItem(it, srcName) {
  ...
  return { title, url, source: srcName, published_at, summary,
           impact, impact_adult: impact,
...
function hoursAgo(d){ const t=new Date(d).getTime(); if(!t) return 1e9; return Math.floor((NOW_MS - t)/36e5); }

function applyConsensusFlexible(rows=[], consensus=2, maxItems=3){
  let out=applyConsensus(rows, consensus, maxItems);
  const tooFew = out.length < Math.min(2, maxItems);
  const tooOld = out[0]?.published_at ? hoursAgo(out[0].published_at) > 12 : true;
  if(tooFew || tooOld){ out=applyConsensus(rows, 1, maxItems); }
  return out;
}

async function fetchFeed(url){
  try{ const feed=await parser.parseURL(url); return feed.items||[]; }
  catch(e){ console.error('Feed error:', url, e.message); return []; }
}
...
async function main(){
  if(!fs.existsSync(SOURCES_PATH)) throw new Error('No existe data/sources.json');
  ...
  async function buildBlock(blockName){
    const def=src[blockName]; if(!def || !Array.isArray(def.feeds)) return [];
    const rows=await collectFromFeeds(def.feeds);
    return applyConsensusFlexible(rows, def.consensus??2, def.maxItems??3);
  }
  ...
  function keepPrevIfEmpty(currentArr, prevKey){
    if((!currentArr || !currentArr.length) && prev && Array.isArray(prev[prevKey]) && prev[prevKey].length){ return prev[prevKey]; }
    return currentArr;
  }
  ...
  const finalOut={...outCompat, blocksOut:{ MolinsDeRei: molins }};

  fs.writeFileSync(OUT_PATH, JSON.stringify(finalOut,null,2),'utf-8');
  fs.writeFileSync(META_PATH, JSON.stringify({version:BUILD_VERSION,builtAt:new Date().toISOString(),commit:GIT_SHA},null,2),'utf-8');
